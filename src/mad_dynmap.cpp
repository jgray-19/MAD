...
inline void misalignexi (cflw<M> &m)
{
  ...
  mad_mat_rtbar(rb, tb, ld, fval(m.mang), fval(m.tlt), m.rot ? r:0, t);

  if (m.rot && m.sdir > 0) {
    num_t v[3];
    mad_mat_torotyxz(rb, v, true); // Returns negative of the angles
    srotation<M>(m, 1, R(m.dpsi)-(a[2]+v[2])); // No edir as weighting handled beforehand
    xrotation<M>(m, 1, R(m.dphi)-(a[0]+v[0]));
    yrotation<M>(m, 1, R(m.dthe)-(a[1]+v[1]));
  }

  if (m.trn) translate<M>(m, -1,
     m.edir*(R(m.dx)-(t[0]+tb[0])), m.edir*(R(m.dy)-(t[1]+tb[1])), R(m.ds)-(t[2]+tb[2]));  // Translation needs to be unweighted due to weighting in translation map

  if (m.rot && m.sdir < 0) {
    num_t v[3];
    mad_mat_torotyxz(rb, v, true); // Returns negative of the angles
    yrotation<M>(m, 1, R(m.dthe)-(a[1]+v[1]));// No edir as weighting handled beforehand
    xrotation<M>(m, 1, R(m.dphi)-(a[0]+v[0]));
